...
app = Flask(__name__)
app.secret_key = "nimbus_emergency_fix"
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nimbus_erp.db")

def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        c = conn.cursor()
        # Siguraduhin na ang tables ay laging kumpleto
        c.execute("CREATE TABLE IF NOT EXISTS properties (id INTEGER PRIMARY KEY, name TEXT)")
        c.execute("CREATE TABLE IF NOT EXISTS tenants (id TEXT PRIMARY KEY, name TEXT, bname TEXT, prop_id INTEGER, t_type TEXT, rent REAL)")
        c.execute("""CREATE TABLE IF NOT EXISTS ledger (id INTEGER PRIMARY KEY AUTOINCREMENT, t_id TEXT, period TEXT, 
                     basic REAL, vat REAL, wtax REAL, signage REAL, electric REAL, water REAL, total REAL, 
                     check_no TEXT, ds_series TEXT, date_paid TEXT, remarks TEXT)""")
        
        # Default Properties (Kailangan ito para sa dropdown)
        props = [(1,'Kapasigan'),(2,'Rosario'),(3,'Marikina'),(4,'Camarin 1'),(5,'Camarin 3'),(6,'Palatiw'),
                 (7,'MB Old'),(8,'MB New'),(9,'Tagaytay 1'),(10,'Tagaytay 2'),(11,'Camarin 2'),(12,'Pateros')]
        c.executemany("INSERT OR IGNORE INTO properties VALUES (?,?)", props)
        conn.commit()
...
<style>
    ...
    .sidebar { width: 200px; background: #2c3e50; color: white; height: 100vh; position: fixed; padding: 20px; }
    ...
    .btn { background: #3498db; color: white; padding: 8px 12px; border: none; border-radius: 4px; cursor: pointer; text-decoration: none; }
...
<!DOCTYPE html>
<html>
<head><title>Nimbus ERP v31</title>{{ css|safe }}</head>
<body>
    <div class="sidebar">
        <h2>NIMBUS</h2>
        <a href="/">📊 Master Ledger</a>
    ...
    <div class="main">
        {% if p == 'dash' %}
            ...
            <div class="card">
                <table>
                    <tr><th>Period</th><th>Tenant</th><th>Total Paid</th><th>Check No</th><th>Date</th></tr>
                    {% for r in data %}
                    <tr><td>{{r[2]}}</td><td><b>{{r[14]}}</b></td><td>₱{{"%.2f"|format(r[9])}}</td><td>{{r[10]}}</td><td>{{r[12]}}</td></tr>
                    {% endfor %}
                </table>
            </div>
        {% elif p == 'dir' %}
            ...
            <div class="card">
                <table>
                    <tr><th>Name</th><th>Type</th><th>Rent</th><th>Action</th></tr>
                    {% for t in tenants %}
                    <tr><td><b>{{t[1]}}</b></td><td>{{t[4]}}</td><td>₱{{t[5]}}</td><td><a href="/add_l/{{t[0]}}" class="btn">Post Payment</a></td></tr>
                    {% endfor %}
                </table>
            </div>
        {% elif p == 'enr' %}
            <div class="card" style="max-width:400px; margin:auto;">
                ...
                <form action="/save_t" method="POST">
                    ...
                    <button type="submit" class="btn" style="width:100%; margin-top:10px;">Save</button>
                </form>
            </div>
        {% elif p == 'add_l' %}
            <div class="card" style="max-width:500px; margin:auto;">
                ...
                <form action="/save_l" method="POST">
                    <input type="hidden" name="tid" value="{{t[0]}}">
                    Period: <input name="period" required placeholder="Month Year">
                    Basic Rent: <input name="basic" type="number" step="0.01" value="{{t[5]}}">
                    Signage: <input name="signage" type="number" step="0.01" value="0">
                    Electric: <input name="elec" type="number" step="0.01" value="0">
                    Water: <input name="water" type="number" step="0.01" value="0">
                    Check No: <input name="check_no" placeholder="N/A">
                    DS Series: <input name="ds_series" placeholder="0001">
                    Date Paid: <input type="date" name="d_paid" required>
                    Remarks: <input name="remarks">
                    <button type="submit" class="btn" style="width:100%; margin-top:10px;">Post Payment</button>
                </form>
            </div>
        {% endif %}
    </div>
...
def save_t():
    ...
    conn.execute("INSERT INTO tenants (id, name, prop_id, t_type, rent) VALUES (?,?,?,?,?)",
               (str(uuid.uuid4())[:8], f['name'], f['prop_id'], f['t_type'], f['rent']))
...
def save_l():
    ...
    total = (basic + vat + float(f['signage']) + float(f['elec']) + float(f['water'])) - wtax
    conn = sqlite3.connect(DB_PATH)
    conn.execute("INSERT INTO ledger (t_id, period, basic, vat, wtax, signage, electric, water, total, check_no, ds_series, date_paid, remarks) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                 (f['tid'], f['period'], basic, vat, wtax, f['signage'], f['elec'], f['water'], total, f['check_no'], f['ds_series'], f['d_paid'], f['remarks']))
    conn.commit(); conn.close()
    return redirect('/')
